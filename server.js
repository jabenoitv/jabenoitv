...
// Crear archivos de configuración desde variables de entorno
...
// Almacenar logs recientes en memoria
const logs = [];
const MAX_LOGS = 300;
let tasksDetected = 0;
...
// Iniciar CashClaw como proceso hijo capturando su output
...
<head>
  ...
  <title>CashClaw · Agente ${process.env.AGENT_ID}</title>
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#0f172a;color:#e2e8f0;min-height:100vh}
    header{background:#1e293b;padding:20px 24px;border-bottom:1px solid #334155;display:flex;align-items:center;gap:12px}
    ...
    .card{background:#1e293b;border-radius:12px;padding:18px 20px;flex:1;min-width:160px;border:1px solid #334155}
    .card label{font-size:.72em;color:#94a3b8;text-transform:uppercase;letter-spacing:.5px}
    .card .val{font-size:1.5em;font-weight:700;margin-top:6px;color:#f1f5f9}
    .card .val.mono{font-size:.9em;font-family:monospace;color:#94a3b8}
    ...
    .logs-hdr{padding:14px 20px;border-bottom:1px solid #334155;font-size:.82em;color:#94a3b8;display:flex;justify-content:space-between;align-items:center}
...
<body>
  ...
  <div class="cards">
    <div class="card"><label>Estado</label><div class="val" id="st"><span class="dot"></span>Cargando...</div></div>
    ...
    <div class="card"><label>Última actividad</label><div class="val" id="la" style="font-size:.9em">-</div></div>
  </div>
  ...
  <div class="footer">Actualización automática cada 15 seg · <a href="/">Actualizar ahora</a></div>
  ...
  <script>
    ...
    async function load(){
      ...
      el.innerHTML=ls.length?[...ls].reverse().map(l=>'<div class="log'+(l.type==="error"?' err':'')+'"><span class="t">'+ftime(l.time)+'</span><span class="msg">'+l.msg.replace(/</g,'&lt;')+'</span></div>').join('')
        :'<div class="empty">Sin actividad aún — polling cada 30 segundos</div>';
    }
    load();
